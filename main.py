...
class MeshWorker(QThread):
    finished = pyqtSignal(object) # Returns PyVista Mesh or None
    ...
    def run(self):
        try:
            ...
            # Size Check (Skip Heavy Processing for > 50MB)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # DELAY INITIAL LOAD to ensure window shows up
        if self.render_file.exists():
            QTimer.singleShot(1000, lambda: self.reload_mesh(str(self.render_file)))
    ...
    def on_process_finished(self, result):
        ...
        # Reload triggers automatically by file watcher usually, but ensure it runs
        if self.render_file.exists():
    ...
    def reload_mesh(self, path):
        # Queuing Logic: If locked, set pending and return
        if hasattr(self, 'mesh_worker') and self.mesh_worker.isRunning():
            self.pending_mesh_path = path
            self.log("   -> Worker busy, queuing reload...", C_TEXT_DIM)
            return

        self.pending_mesh_path = None # Clear pending since we are starting
        self.log(f"Loading Mesh: {Path(path).name}...", C_ACCENT_B)
        
        self.mesh_worker = MeshWorker(path)
    ...
    def on_mesh_ready(self, mesh):
        try:
            ...
            self.log(f"Render Complete ({mesh.n_points} pts).", "#00FF00")
            
            # CHECK QUEUE
            if getattr(self, 'pending_mesh_path', None):
                next_path = self.pending_mesh_path
                self.pending_mesh_path = None
                # Small delay to let UI breathe
                QTimer.singleShot(100, lambda: self.reload_mesh(next_path))
            
        except Exception as e:
            self.log(f"Display Error: {e}", "#FF0000")

    def on_mesh_error(self, msg):
        self.log(f"Render Error: {msg}", "#FF0000")
